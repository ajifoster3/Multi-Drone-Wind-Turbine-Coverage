...
int main(int argc, char ** argv)
{
	...
	if (argc < 2) {
		RCLCPP_ERROR(
			rclcpp::get_logger(
				"rclcpp"), "Usage: ros2 run [package_name] [executable_name] [team_size]");
		rclcpp::shutdown();
		return 1;
	}
	...
	auto central_coverage_controller_node = std::make_shared<CentralCoverageControllerNode>(
		"central_coverage_controller_node", team_size);
	...
	for (int i = 1; i <= team_size; i++) {
		...
		while (!rclcpp::wait_for_message(
			       geoMsg, central_coverage_controller_node,
			       "central_control/uas_" + uasNumber + "/global_pose", std::chrono::seconds(10)))
		{
			RCLCPP_INFO(
				rclcpp::get_logger("rclcpp"), "waiting for message from uas_%s",
				uasNumber.c_str());
		}
	...
	while (!rclcpp::wait_for_message(
		       clockMsg, central_coverage_controller_node, "clock",
