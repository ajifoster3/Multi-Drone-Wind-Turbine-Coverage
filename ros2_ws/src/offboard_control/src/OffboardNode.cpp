...
void OffboardNode::initializeSubscribers()
{
	stateSub_ = this->create_subscription<mavros_msgs::msg::State>(
		"mavros/uas_" + std::to_string(uasNumber_) + "/state", 10,
		std::bind(&OffboardNode::stateCb, this, std::placeholders::_1));
	globalPosSub_ = this->create_subscription<sensor_msgs::msg::NavSatFix>(
		"mavros/uas_" + std::to_string(uasNumber_) + "/global_position/global",
		rclcpp::SensorDataQoS(),
		std::bind(&OffboardNode::globalPositionCb, this, std::placeholders::_1));
	centralGoalPosSub_ = this->create_subscription<geographic_msgs::msg::GeoPoseStamped>(
		"central_control/uas_" + std::to_string(uasNumber_) + "/goal_pose",
		rclcpp::SensorDataQoS(),
		std::bind(&OffboardNode::globalGoalPositionCb, this, std::placeholders::_1));
}
